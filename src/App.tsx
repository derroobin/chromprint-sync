...
const asyncAudio = async (duration: number) => {
  ...
  return {
    ...
    data: JSON.stringify(Array.from(full)),
  };
};
...
function App() {
  ...
  return (
    <div className="App">
      ...
      <div>
        <animated.span>
          {time.content.to((x) => x.toFixed(0) + "s")}
        </animated.span>
      </div>
      {import.meta.env.DEV ? (
        <h3 style={{ color: "white" }}>
          {count === 0 ? " calculating..." : count + "s"}
        </h3>
      ) : null}
      {data && (
        <div>
      ...
      <div>
        on the first click this might sync, because i dont get the permission to
        use audio first
      </div>
